...
// Add Zen Tabs styles
if (theme_get_setting('new_theme_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'new_theme') . '/css/tabs.css');
}
 ...
 *   Template variables.
 */
function new_theme_preprocess_html(&$vars) {
  if (!module_exists('conditional_styles')) {
    new_theme_add_conditional_styles();
  }
...
function new_theme_preprocess_page(&$vars, $hook) {
...
function new_theme_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function new_theme_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('new_theme_breadcrumb');
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {



    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('new_theme_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }
    // Return the breadcrumb with separators.
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('new_theme_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('new_theme_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('new_theme_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function new_theme_add_conditional_styles() {
 ...
 */
function new_theme_preprocess_menu_local_task(&$variables) {
...
function new_theme_menu_local_tasks(&$variables) {
  $output = '';
